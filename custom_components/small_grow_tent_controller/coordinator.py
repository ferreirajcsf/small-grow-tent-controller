...
class GrowTentCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def _async_switch(self, entity_id: str, turn_on: bool, blocking: bool = False) -> None:
        """Call switch.turn_on / turn_off.

        blocking=True is reserved for safety-critical trips (heater max-run lockout)
        to guarantee the command completes before control logic continues.
        """
    ...
    async def _apply_control(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(light_on, time):
            light_on = _DEFAULT_LIGHT_ON
        if not isinstance(light_off, time):
            light_off = _DEFAULT_LIGHT_OFF
        ...
        if not enabled:
            ...
            if exhaust_mode_sel != "Auto" and exhaust_eid:
                ...
                if (
                    ...
                    avg_rh_val = float(data["avg_rh"])
                    if (max_t is not None and avg_t >= float(max_t)) or (max_rh is not None and avg_rh_val >= float(max_rh)):
                        desired = True
        ...
        # Read current device states BEFORE applying forced overrides,
        # so heater on-time tracking sees the real hardware state.
        ...
        # --- HEATER ON-TIME TRACKING (uses actual hardware state, not post-override) ---
        if heater_on_actual:
        ...
        if heater_on_actual and self.control.heater_on_since:
        ...
        # ----------------------------
        # HEATER SAFETY: max continuous run time
        # blocking=True here to guarantee the safety trip completes before anything else.
        # ----------------------------
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        data: dict[str, Any] = {
            ...
            "light_on_time": self._parse_time(light_on_s, _DEFAULT_LIGHT_ON),
            "light_off_time": self._parse_time(light_off_s, _DEFAULT_LIGHT_OFF),
            "control_mode": "init",
